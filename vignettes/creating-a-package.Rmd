...
Sometimes there may be files that you don't want included in your R package. For example, I have two files, `originstructions.Rmd` and `originstructions.html`, that I had originally used to document the steps  for creating a package; however, I have since moved that information to this vignette, so I do not want these original files to be included in the R package.

```{r, echo = FALSE, eval = FALSE}
# ignore original instructions files (moved to vignettes)
use_build_ignore("originstructions.html")
use_build_ignore("originstructions.Rmd")
